...
describe('CroDraw', function () {
  let TRPZFactory,
    LotteryFactory,
    WitnetRequestBoardFactory,
    WitnetRandomnessFactory
  ...
  before(async () => {
    ...
    WitnetRandomnessFactory = await ethers.getContractFactory(
      'WitnetRandomness'
    )
  })
  ...
  it('Distribute rewards to users', async function () {
    ...
    witnetRandomness = await WitnetRandomnessFactory.deploy(
    ...
    await (
      await lottery.setOperatorAndTreasuryAndCharityAddresses(
        operator.address,
        project.address,
        charity.address,
        founder1.address,
    ...
    for (i = 0; i < 100; ++i) {
      await (
        await lottery
          .connect(accounts[i])
          .buyTickets(1, { from: accounts[i].address, value: 1000 })
      ).wait()
    }
    let accountBalances = []
    for (i = 0; i < 100; ++i) {
    ...
    await (
      await lottery
        .connect(operator)
        .closeLottery({ from: operator.address, value: '10' })
    ).wait()
